    ...
    private inner class FlowEngine : Engine() {
        ...
        private fun startDrawing() {
            ...
            thread = Thread {
                ...
                while (running) {
                    ...
                    val sleepMs = (16L - elapsedMs).coerceAtLeast(1L)
        ...
        private fun drawFrame(t: Float) {
            ...
            try {
                ...
                // Deep, subtle animated background.
                ...
                // Slowly moving luminous orbs.
                drawOrb(canvas, w * 0.23f + cos(t * 0.52f) * w * 0.16f,
                    h * 0.30f + sin(t * 0.67f) * h * 0.13f, minDim * 0.34f,
                    Color.argb(90, 40, 145, 255))
                drawOrb(canvas, w * 0.76f + sin(t * 0.43f) * w * 0.13f,
                    h * 0.58f + cos(t * 0.58f) * h * 0.16f, minDim * 0.38f,
                ...
                for (ribbon in 0..3) {
                    ...
                    while (x <= w * 1.05f) {
                        val nx = x / w
                        val y = baseY +
                            sin(nx * 6.4f + t * (0.75f + ribbon * 0.08f)) * h * 0.045f +
                            cos(nx * 3.1f - t * 0.45f) * h * 0.025f
                        path.lineTo(x, y)
                        x += w / 80f
                    }
                ...
                // Fine star-like particles.
                paint.shader = null
                paint.style = Paint.Style.FILL
                for (i in 0 until 28) {
                    val px = ((i * 83.0 + t * (7 + i % 4) * 4) % (w + 40)) - 20
                    val py = ((i * 137.0 + sin(t * 0.3f + i) * 18) % (h + 40)) - 20
                    val radius = minDim * (0.0015f + (i % 3) * 0.001f)
                    paint.color = Color.argb(90 + (i % 4) * 30, 180, 215, 255)
                    canvas.drawCircle(px.toFloat(), py.toFloat(), radius, paint)
                }
            } finally {
        ...
        private fun drawOrb(canvas: Canvas, cx: Float, cy: Float, radius: Float, color: Int) {
            ...
            p.shader = RadialGradient(
                cx, cy, radius,
                intArrayOf(color, Color.argb(20, Color.red(color), Color.green(color), Color.blue(color)), Color.TRANSPARENT),
